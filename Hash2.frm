...
Public raktar$, cikk$
Public Function GyariszamKer(rakt$, cikk$, Gyariszam$, tipus$) As String
  gyrec$ = ""
  keresobj$ = "KKFX"
  ...
  Next
   If resdb& > 0 Then
      ...
      For i97& = 1 To resdb&
         ...
         If Trim(xkonver(Mid$(r$, 200, 40))) = Gyariszam$ Then
            Bztip$ = Mid$(r$, 55, 2)
            hivatkozas$ = Trim(Mid$(r$, 57, 10))
            rraktar$ = Mid$(r$, 8, 4)
            rcikk$ = Trim(Mid$(r$, 12, 15))
            If tipus$ = "BS" Then
               hivatkozas$ = ""
            End If
            ' csak a bent lévõ gyáriszámokat kell kiírni
            If Bztip$ = "BS" And hivatkozas$ = "" And rraktar$ = rakt$ And rcikk$ = Trim(cikk$) Then
                gyrec$ = r$
            End If
         End If
      Next
...
Private Sub Command21_Click()
  If colwimod% = 0 Then
    For izu% = 1 To MSFlexGrid1.Cols
      hxh% = Hash2.TextWidth(Trim(MSFlexGrid1.TextMatrix(0, izu% - 1)))
      hxk% = Hash2.TextWidth(String(kmho%(izu%), " "))
...
Private Sub Form_Resize()
  Hash2.Width = 6000
  MSFlexGrid1.Width = Hash2.Width - 825
  Command1.Left = Hash2.Width - 2960
  Command2.Left = Hash2.Width - 1520
  Command21.Left = Hash2.Width - 600
  Command22.Left = Hash2.Width - 600
  Command23.Left = Hash2.Width - 600
  Command24.Left = Hash2.Width - 600
  Command25.Left = Hash2.Width - 600
  Command26.Left = Hash2.Width - 600
  Text1.Width = Hash2.Width - 300
End Sub
...
Private Sub megmutat(dbxn$, indn$)
  ...
  For i& = 1 To sdb%
    ...
    If spz& > 0 Then
      ...
      hivatkozas$ = Trim(Mid$(r$, 57, 10))
      rraktar$ = Mid$(r$, 8, 4)
      sztornojel$ = Mid$(r$, 320, 1)
      rcikk$ = Mid$(r$, 12, 15)
      ' csak a bent lévõ gyáriszámokat kell kiírni
      If Bztip$ = "BS" And hivatkozas$ = "" And rraktar$ = Hash2.raktar$ And Trim(rcikk$) = Trim(Hash2.cikk) And sztornojel$ = " " Then
        If elso& = 0 Then
           elso& = spz&
        End If
        For i2& = 1 To kmesor%(0)
          adamez$ = Mid$(r$, kmkp%(i2&), kmho%(i2&))
          MSFlexGrid1.TextMatrix(j&, i2& - 1) = adamez$
        Next
        j& = j& + 1
        If j& > lapmeret% Then
           Exit For
  ...
  inex& = aktsor&
  If statusz% = 0 Then spz& = inex& Else spz& = res&(inex&)
  ' Az Elõzõ tételt választja ki
  If spz& > 0 Then
    kkfxkulcs$ = Trim(MSFlexGrid1.TextMatrix(MSFlexGrid1.Row, 2))
    r$ = dbxkey("KKFX", kkfxkulcs$)
    'Get #ndfi, (spz& - 1) * ih& + 1, rcim&
    '--- rekordok beallitasa es kitoltes
    'Seek #dbfi, rcim& + 4
...
Private Sub Form_Load()
  ...
  Text1.Text = ""
  Hash2.Caption = "Gyártási számok keresése"
  ...
  If hkh% > 0 Then
    Select Case hkh%
      ...
      Case 3
        resdb& = 0
        For i97& = 1 To Len(hst1$)
           aaa$ = Mid$(hst1$, i97&, 1) + Mid$(hst2$, i97&, 1) + Mid$(hst3$, i97&, 1)
          If hk$ = Mid$(hst1$, i97&, 1) + Mid$(hst2$, i97&, 1) + Mid$(hst3$, i97&, 1) Then
            If resdb& < 30000 Then
              resdb& = resdb& + 1: res&(resdb&) = i97&
            Else
